...
\item to offer ideas about how to enhance \textit{reusability} of existing and future systems of this kind.
\end{itemize}
...
While this approach to distributes computing won't work well for most experiments of the LHC scale (where moving significant amounts of data presents a perennial problem)
...
problems prior to actual job dispatch can be identified and flagged at both site and worker node level. This eliminates a very large fraction of potential failures
that the user would otherwise have to deal with and account for, since the resource pool exposed to the user (or for an automated client performing job submission)
...
communications with the server, downloading the payload applications, logging and others.
