...
The ability of the user or operator of a WMS to gain immediate and efficient access to information describing the status of jobs, tasks and operations performed on the data
...
in the system (units of data, jobs, tasks etc). This is made possible by utilizing a database to keep the sates of these objects.
...
expertise and maintenance policies at the target organization and in some cases even cost (cf. Oracle). Solutions such as an ORM layer
should be utilized to make possible utilization of a variety of database products as the back-end for the Workload Management System,
...
widely used as a crucial factor in managing the workload distribution in HEP domain in ``near time''. In this approach, network perfromance information
...
% A complementary strategy is application of ``Intelligent Networks'', whereby 
