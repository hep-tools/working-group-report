...
We begin with an observation that in a few examples of WMS there is a concept of a \textbf{task} which is a collection or grouping of jobs either created from the same template
and differening only by some input parameters, or grouped together based on their interdependence and purpose (this is not a universal definition and it does clash with ``tasks'' used
in the batch system context but we'll stick with with in this chapter since it's widely used). The task in this case is a rudimentary case of workflow management tool.

The ability of the user or operator of WMS to get immediate and efficient access to information describing the status of jobs and tasks is an essential feature of a good Workload Management System~\cite{pandamon_chep10}.
